...
function ModalDeletarFuncionario({ aberto, fechado, funcionario, onDeletar }: Props) {
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[110] p-4">
            <div className="max-w-sm w-full bg-white rounded-3xl p-8 text-center shadow-2xl relative">
                ...
                <p className="text-gray-500 mb-8 text-sm px-2">
                    Você tem certeza que deseja remover <strong>{funcionario.nome}</strong> da equipe?
                </p>
                <div className="flex gap-3">
                    ...
                    <button
                        onClick={async () => {
                            const resposta = await FuncServ.delete(funcionario.usuario)
                            if (resposta.status !== 'sucess') return
                            onDeletar()
                        }}
