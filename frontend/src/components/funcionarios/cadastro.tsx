...
import { createFuncionario } from '../../types/funcionario';
...
function FormCadastroFuncionario({ aberto, fechado, onSalvar, onErro }: Props) {
    const [form, setForm] = useState<createFuncionario>({
        nome: '', telefone: '', endereco: '', usuario: '', senha: '', nivelPermissao: 'OPERADOR'
    })

    const mudanca = (campo: string, valor: string) => setForm(prev => ({ ...prev, [campo]: valor }))
            ...
            <div className="max-w-2xl w-full bg-white rounded-3xl p-8 relative shadow-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                    <div className="md:col-span-2">
                        ...
                        <input type="text" placeholder="Nome do colaborador" value={form.nome} onChange={(e) => mudanca('nome', e.target.value)}
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="ex: hugo.santos" value={form.usuario} onChange={(e) => mudanca('usuario', e.target.value)}
                    ...
                    <div>
                        ...
                        <input type="password" placeholder="••••••••" value={form.senha} onChange={(e) => mudanca('senha', e.target.value)}
                            className="w-full mt-1 p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-[#123354] transition-all" />
                    </div>
                    <div>
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Telefone</label>
                        <input type="text" placeholder="(12) 99999-9999" value={form.telefone} onChange={(e) => mudanca('telefone', e.target.value)}
                            className="w-full mt-1 p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-[#123354] transition-all" />
                    </div>
                    <div>
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">Nível de Permissão</label>
                        <select value={form.nivelPermissao} onChange={(e) => mudanca('nivelPermissao', e.target.value)}
                    ...
                    <div className="md:col-span-2">
                        ...
                        <input type="text" placeholder="Rua, número, bairro e cidade" value={form.endereco} onChange={(e) => mudanca('endereco', e.target.value)}
                            className="w-full mt-1 p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-[#123354] transition-all" />
                    </div>
                </div>
                <div className="flex justify-end gap-3 mt-8">
                    <button onClick={fechado} className="px-6 py-2.5 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 font-semibold transition-all">
                        Cancelar
                    </button>
                    <button
                        onClick={async () => {
                            try {
                                onErro(null);
                                await FuncServ.create(form);
                                onSalvar();
                                fechado();
                            } catch (err: any) {
                                onErro(err?.response?.data?.message ?? 'Erro ao cadastrar funcionário.');
                            }
                        }}
