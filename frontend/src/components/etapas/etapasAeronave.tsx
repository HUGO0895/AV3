...
import FormEditarEtapa from './formEditarEtapas';
import FormDeletarEtapa from './formDeletarEtapas';
import { responseEtapa } from '../../types/etapas';

interface EtapaProps {
  aberto: boolean;
  etapas: Array<responseEtapa>;
  onSalvar: () => void;
  onErro: (msg: string) => void;
  aeronave_id: string;
}

function EtapasDaAeronave({ aberto, etapas, onSalvar, onErro, aeronave_id }: EtapaProps) {
  const [modalEditar, setModalEditar] = useState(false);
  const [modalDeletar, setModalDeletar] = useState(false);
  const [etapaSelecionada, setEtapaSelecionada] = useState<responseEtapa | undefined>(undefined);
    ...
    <>
      {etapas.length > 0 ? (
        ...
        <div className="col-span-full py-20 text-center bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
          <p className="text-gray-400 font-bold">Nenhuma etapa encontrada.</p>
        </div>
      )}

      {modalEditar && etapaSelecionada && (
        <FormEditarEtapa
          aberto={modalEditar}
          fechado={() => { setModalEditar(false); setEtapaSelecionada(undefined) }}
      ...
      {modalDeletar && etapaSelecionada && (
        <FormDeletarEtapa
          aberto={modalDeletar}
          fechado={() => { setModalDeletar(false); setEtapaSelecionada(undefined) }}
